...
class VRAMProfile:
    ...
    ----------
    total_vram_gb : float
        Total physical VRAM in GB.
    available_vram_gb : float
        Currently free VRAM in GB.
    model_vram_gb : float
        VRAM reserved/used by the loaded model.
    max_pixels : int
        Calculated absolute maximum pixel count.
    optimal_pixels : int
        Calculated optimal pixel count for stability.
    min_pixels : int
        Minimum allowed pixel count (default 256*28*28).
    resolution_tiers : List[int]
        Fallback tiers for OOM recovery.
    """
...
class ImageProcessingConfig:
    ...
    max_pixels : int
        Maximum resolution ceiling.
    optimal_pixels : int
        Target resolution.
    max_retry_attempts : int
        How many times to downscale on OOM.
    pixels_per_token : int
        Rough estimate of pixels per token for cost calculation.
    """
...
class AdaptiveImageProcessor:
    ...
    def from_gpu_info(cls, model_vram_gb: float = 0) -> "AdaptiveImageProcessor":
        ...
        ----------
        model_vram_gb : float
            Estimated VRAM usage of the loaded model weights.
        ...
        if torch.cuda.is_available():
            ...
            logger.info(
                f"GPU: {total_gb:.1f}GB total, {available_gb:.1f}GB free -> max {vram_profile.max_pixels // 1000}K px"
            )
    ...
    def _calculate_resize_dims(
        ...
        """
        Cached dimension calculation ensuring patch alignment.

        Parameters
        ----------
        original_width, original_height : int
            Input dimensions.
        max_pixels, min_pixels : int
            Target constraints.
        patch_factor : int
            Factor dimensions must be divisible by (usually 28).
    ...
    def smart_resize(
        ...
        """
        Resize image maintaining aspect ratio and patch alignment.
        ...
        max_pixels : int, optional
            Override max pixels from config.
        min_pixels : int, optional
            Override min pixels from config.
        ...
        resample = (
            ...
            if self.config.use_lanczos_resize
    ...
    def prepare_for_inference(
        ...
        image : PIL.Image
            Input image.
        tier_index : int
            Resolution tier to use (0=highest).
        ...
        if self.vram_profile and self.vram_profile.resolution_tiers:
            ...
            max_pixels = int(
                self.config.optimal_pixels * (self.config.downscale_factor**tier_index)
            )
    ...
    def reset_tier(self):
        """Reset the current fallback tier to 0."""
        self._current_tier_index = 0

    def next_tier(self) -> bool:
        """
        Move to the next lower resolution tier.

        Returns
        -------
        bool
            True if a lower tier is available, False otherwise.
        """
        max_tiers = (
            len(self.vram_profile.resolution_tiers)
...
def create_processor_with_dynamic_resolution(
    ...
    vram_gb : float
        Total system VRAM.
    model_loaded_gb : float
        VRAM used by weights.

    Returns
    -------
    Tuple[Any, ImageProcessingConfig]
        Configured processor and config object.
    """
